...
class TestNbConvertApp(TestsBase):
    ...
    @pytest.mark.skipif(
        os.name == "nt", reason="CLI parsing does not work the same on Windows"
    )
    ...
    def test_execute_widgets_from_nbconvert(self):
        ...
        with self.create_temp_cwd([f"{notebookName}.ipynb"]):
            ...
            with open(f"{notebookName}.html", "r", encoding="utf8") as f:
                text = f.read()
                assert (
                    '<script type="application/vnd.jupyter.widget-view+json">' in text
                )
                assert (
                    '<script type="application/vnd.jupyter.widget-state+json">' in text
                )
